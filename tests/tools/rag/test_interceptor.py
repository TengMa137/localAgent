...
class TestWebSearch:
    ...
    @pytest.mark.asyncio
    async def test_calls_mcp_web_search_with_result_limit(self):
        with patch("tools.retrieval.interceptor.fastmcp") as _mock_fmcp:
            _mc = MagicMock()
            _mc.call_tool = AsyncMock(return_value=_mcp_result({"results": []}))
            _mock_fmcp.Client.return_value.__aenter__ = AsyncMock(return_value=_mc)
            _mock_fmcp.Client.return_value.__aexit__ = AsyncMock(return_value=False)

            await web_search_results(
                "http://mcp:8000/sse",
                "test query",
                max_results=3,
            )
    ...
    @pytest.mark.asyncio
    async def test_calls_mcp_weather_forecast(self):
        payload = {"success": True, "query": "Lund, Sweden"}
        with patch("tools.retrieval.interceptor.fastmcp") as _mock_fmcp:
            _mc = MagicMock()
            ...
            result = await weather_forecast_result(
                "http://mcp:8000/sse",
                "Lund, Sweden",
                date="2026-06-06",
            )
        ...
        _mc.call_tool.assert_awaited_once_with(
            "weather_forecast",
            {"location": "Lund, Sweden", "date": "2026-06-06"},
        )

    @pytest.mark.asyncio
    async def test_calls_mcp_wiki_summary(self):
        payload = {"success": True, "title": "Diffusion model"}
        with patch("tools.retrieval.interceptor.fastmcp") as _mock_fmcp:
            _mc = MagicMock()
            _mc.call_tool = AsyncMock(return_value=_mcp_result(payload))
            _mock_fmcp.Client.return_value.__aenter__ = AsyncMock(return_value=_mc)
            ...
            result = await wiki_summary_result(
                "http://mcp:8000/sse",
                "diffusion model",
                language="en",
            )
